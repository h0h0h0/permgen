...
/**
 * Command to encapsulate initial search logic for the key search scenario. The aim is to generate a picklist, where each
...
public class KeySearch implements Command {
    public static final String NAME = "KeySearch";
    ...
    public String execute(HttpServletRequest request,
        ...
        try {
            ...
            searchService.setEngineType(QuickAddress.KEYFINDER);
            searchService.setFlatten(true);
            
            Picklist picklist;
            if (bInitialSearch) {
                // Perform the initial search (key engine, keytened picklists)
            ...
            } else if ( picklist.getItems() == null || picklist.getTotal() == 0) {
                ...
                if ( bInitialSearch && ( picklist.isAutoFormatSafe() || picklist.isAutoFormatPastClose() ) ) {
                    ...
                    // Go format page (sDestinationPage is already set appropriately)
                                ...
                                else if ( bInitialSearch && ( picklist.getItems().length == 1 ) && mustRefine( picklist.getItems()[0] ) )
                                {
                                        ...
                                        request.setAttribute(Constants.MONIKER, picklist.getItems()[0].getMoniker());
                                        Command refine = new KeyRefineAddress();
                    ...
                    for (int i = 0; i < iSize; i++) {
                       ...
                       /* if (mustRefine(item))
                        {
                            asCommands[i] = KeyRefineAddress.NAME;
                        }
                        else
                        {*/
                        asCommands[i] = KeyFormatAddress.NAME;
                        //}
                    }
                    ...
                    // display page
                    sDestinationPage = Constants.KEY_SEARCH_PAGE;
                }
            }
        } catch (Exception e) {
            // dump the exception to error stream
            System.err.println("~~~~~~~~~ Caught exception in Key Search command ~~~~~~~~~~");
            e.printStackTrace();
            System.err.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
